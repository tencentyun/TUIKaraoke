...
public class KaraokeRoomAudienceActivity extends KaraokeRoomBaseActivity {
    ...
    public void startTakeSeat(final int itemPos) {
        ...
        if (mNeedRequest) {
            ...
            String inviteId = mTRTCKaraokeRoom.sendInvitation(TCConstants.CMD_REQUEST_TAKE_SEAT, mOwnerId, String.valueOf(changeSeatIndexToModelIndex(itemPos)), new TRTCKaraokeRoomCallback.ActionCallback() {
                @Override
                public void onCallback(int code, String msg) {
                    if (code == 0) {
                        ToastUtils.showShort(R.string.trtckaraoke_toast_application_has_been_sent_please_wait_for_processing);
                    } else {
                        ToastUtils.showShort(getString(R.string.trtckaraoke_toast_failed_to_send_application, msg));
                    }
                }
            });
