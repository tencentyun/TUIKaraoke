package com.tencent.liteav.tuikaraoke.ui.lrc.widget;
...
import com.tencent.liteav.tuikaraoke.ui.lrc.utils.LyricsUtils;
...
public abstract class AbstractLrcView extends View {
    ...
    private void init(Context context) {
        ...
        //初始默认数据
        mLoadingText = context.getString(R.string.trtckaraoke_room_id);
        mLoadErrorText = context.getString(R.string.trtckaraoke_room_id);
        mNonsupportText = context.getString(R.string.trtckaraoke_room_id);
        mGotoSearchText = context.getString(R.string.trtckaraoke_room_id);
...
//                LyricsUtils.drawText(canvas, mPaint, mPaintColors, text, x, y);
    ...
    public String getLineLrc(int playProgress) {
        ...
        return LyricsUtils.getLineLrc(mLyricsReader.getLyricsType(), mLrcLineInfos,
    ...
    public void updateSplitData(long playProgress) {
        ...
        if (mLyricsReader.getLyricsType() == LyricsInfo.DYNAMIC) {
            //获取分割后的索引
            mSplitLyricsLineNum = LyricsUtils.getSplitDynamicLyricsLineNum(mLrcLineInfos,
            ...
            mLyricsWordIndex = LyricsUtils.getLyricsWordIndex(mLrcLineInfos, mLyricsLineNum,
                    playProgress, mLyricsReader.getPlayOffset());
            //获取分割后的歌词字索引
            mSplitLyricsWordIndex = LyricsUtils.getSplitLyricsWordIndex(mLrcLineInfos,
                    mLyricsLineNum, playProgress, mLyricsReader.getPlayOffset());
