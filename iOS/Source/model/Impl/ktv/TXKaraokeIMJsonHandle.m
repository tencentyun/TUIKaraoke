...
+ (NSDictionary<NSString *,NSString *> *)getInitRoomDicWithRoomInfo:(TXKaraokeRoomInfo *)roominfo seatInfoList:(NSArray<TXKaraokeSeatInfo *> *)seatInfoList{
    ...
    [result setValue:Karaoke_KEY_ATTR_VERSION forKey:Karaoke_VALUE_ATTR_VERSION];
    NSString *jsonRoomInfo = [roominfo mj_JSONString];
    [result setValue:jsonRoomInfo forKey:Karaoke_KEY_ROOM_INFO];
    for (int index = 0; index < seatInfoList.count; index += 1) {
        NSString *jsonInfo = [seatInfoList[index] mj_JSONString];
        NSString *key = [NSString stringWithFormat:@"%@%d", Karaoke_KEY_SEAT, index];
...
+ (NSDictionary<NSString *,NSString *> *)getSeatInfoListJsonStrWithSeatInfoList:(NSArray<TXKaraokeSeatInfo *> *)seatInfoList {
    ...
    [seatInfoList enumerateObjectsUsingBlock:^(TXKaraokeSeatInfo * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *key = [NSString stringWithFormat:@"%@%lu", Karaoke_KEY_SEAT, (unsigned long)idx];
...
+ (NSDictionary<NSString *,NSString *> *)getSeatInfoJsonStrWithIndex:(NSInteger)index info:(TXKaraokeSeatInfo *)info {
    ...
    NSString *key = [NSString stringWithFormat:@"%@%ld", Karaoke_KEY_SEAT, (long)index];
    [result setValue:json forKey:key];
    return result;
}

+ (TXKaraokeRoomInfo *)getRoomInfoFromAttr:(NSDictionary<NSString *,NSString *> *)attr {
    NSString *jsonStr = [attr objectForKey:Karaoke_KEY_ROOM_INFO];
...
+ (NSString *)getRoomdestroyMsg {
    NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithCapacity:2];
    [result setValue:Karaoke_VALUE_ATTR_VERSION forKey:Karaoke_KEY_ATTR_VERSION];
    [result setValue:@(kKaraokeCodeDestroy) forKey:Karaoke_KEY_CMD_ACTION];
...
+ (NSString *)getCusMsgJsonStrWithCmd:(NSString *)cmd msg:(NSString *)msg {
    NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithCapacity:2];
    [result setValue:Karaoke_VALUE_ATTR_VERSION forKey:Karaoke_KEY_ATTR_VERSION];
    [result setValue:@(kKaraokeCodeCustomMsg) forKey:Karaoke_KEY_CMD_ACTION];
    [result setValue:cmd forKey:Karaoke_KEY_INVITATION_CMD];
...
+ (NSDictionary<NSString *,NSString *> *)parseCusMsgWithJsonDic:(NSDictionary *)jsonDic {
    NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithCapacity:2];
    result[@"cmd"] = [jsonDic objectForKey:Karaoke_KEY_INVITATION_CMD] ?: @"";
...
+ (NSDictionary *)makeInstruction:(NSString *)cmd roomID:(int)roomID {
    NSDictionary *result = @{
        ...
        Karaoke_KEY_CMD_DATA : @{
            Karaoke_KEY_CMD_ROOMID : @(roomID),
            Karaoke_KEY_CMD_INSTRUCTION : cmd
        }
    };
    return result;
}
