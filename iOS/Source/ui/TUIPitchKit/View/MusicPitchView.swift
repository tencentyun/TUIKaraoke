...
class MusicPitchView: UIView {
    ...
    private lazy var bgImageView: UIImageView = {
        let imageView = UIImageView(image: UIImage(named: "score_bg", in: karaokeBundle(), with: nil))
        return imageView
    }()
    ...
    func setScore(score: Int) {
        ...
        scoreLabel.alpha = 1
        scoreLabel.text = localizeReplaceXX(.singleScoreText, "\(score)")
    }
...
fileprivate extension String {
    static var singleScoreText: String {
        karaokeLocalize("Demo.TRTC.Karaoke.singleScorexx")
    }
